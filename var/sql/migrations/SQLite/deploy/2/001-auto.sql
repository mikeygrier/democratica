...
-- Created on Wed Aug 14 22:17:08 2013
-- 
...
CREATE TABLE meritcommons_user (
  ...
  enc_priv_key varchar(255),
  sign_pub_key varchar(255),
  sign_priv_key varchar(255),
...
CREATE UNIQUE INDEX unique_id ON meritcommons_user (unique_id);
CREATE UNIQUE INDEX userid ON meritcommons_user (userid);
--
...
CREATE TABLE meritcommons_stream_message (
  ...
  subtype varchar(64),
  FOREIGN KEY (regarding) REFERENCES meritcommons_stream_message(unique_id),
  FOREIGN KEY (in_reply_to) REFERENCES meritcommons_stream_message(unique_id) ON DELETE CASCADE ON UPDATE CASCADE,
  FOREIGN KEY (submitter) REFERENCES meritcommons_user(id)
);
...
CREATE TABLE meritcommons_user_group (
  ...
  common_name varchar(255) NOT NULL,
  owner integer NOT NULL,
  FOREIGN KEY (owner) REFERENCES meritcommons_user(id)
);
