...
-- Created on Mon Mar 16 16:55:41 2015
-- 
...
CREATE TABLE meritcommons_stream (
  ...
  membership_requires_moderator_approval integer NOT NULL DEFAULT 0,
...
CREATE TABLE meritcommons_file (
  ...
  FOREIGN KEY (uploader) REFERENCES meritcommons_user(id)
);
...
CREATE TABLE meritcommons_stream_invite (
  ...
  create_time integer NOT NULL,
  FOREIGN KEY (invitee) REFERENCES meritcommons_user(id) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE meritcommons_stream_message (
  ...
  original_body text,
  subject text,
  nag_interval integer NOT NULL DEFAULT 0,
  submitter_mask text,
  body text NOT NULL,
...
CREATE TABLE meritcommons_file_variant (
  id INTEGER PRIMARY KEY NOT NULL,
  common_name varchar(255) NOT NULL DEFAULT 'original',
  storage_type varchar(255) NOT NULL DEFAULT 'default',
  ...
  create_time integer NOT NULL,
  modify_time integer NOT NULL,
  FOREIGN KEY (file) REFERENCES meritcommons_file(id) ON UPDATE CASCADE
);
