...
package MeritCommons::Model::Stream::ChangeLog;

use base qw/DBIx::Class/;

__PACKAGE__->load_components(qw/PK::Auto Core/);
__PACKAGE__->table('meritcommons_stream_changelog');
...
sub sqlt_deploy_hook {
    ...
    $sqlt_table->add_index(
        name   => 'stream_changelog_undo_id_idx',
        fields => ['undo_id'],
    );

    $sqlt_table->add_index(
        name   => 'stream_changelog_create_time_idx',
        fields => ['create_time'],
    );

    $sqlt_table->add_index(
        name   => 'stream_changelog_stream_idx',
        fields => ['stream'],
    );
}
