#
# Access Logging using syslog
#

package MeritCommons::SyslogAccessLog;
        ...
        around_dispatch => sub {
            ...
            $c->tx->on(
                finish => sub {
                    ...
                    if (my $ar = $app->config->{flock_syslog}) {
                        ...
                        for (my $i = 0 ; $i <= $#{$ar} ; $i++) {
                            ...
                            unless ($logger =
                                ref $self->{flock_sysloggers} eq "ARRAY" && $self->{flock_sysloggers}->[$i]) {
                                $logger = $self->{flock_sysloggers}->[$i] =
                                  Log::Syslog::Fast->new(LOG_UDP, $ar->[$i], 514, LOG_LOCAL3, LOG_NOTICE,
                                    $app->config->{front_door_host}, 'access_log');
                            }

                            eval {
                                $logger->send(
                                    "@{[$c->instance_id]} " . _log($c, $format, \@handler, $t0 ? tv_interval($t0) : ()),
                                    time
                                );
                            };
                        }
