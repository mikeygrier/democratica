...
// let jquery drive the show onReady.
require([
    'jquery',
    'backbone_components/views/common/navbar',
    'backbone_components/views/common/sidebar'  
], function($, Navbar, Sidebar) {
    $(function() {
        // keep track of focus on the window.
        window.in_focus = true; // when the page loads, it's usually in focus.
        
        $(window).bind('blur', function(e) {
            window.in_focus = false;
        });
        ...
        if (loadJsView != undefined) {
            loadJsView();
        }

        if (typeof window.afterMain === 'object' && window.afterMain.length > 0) {
            $.each(window.afterMain, function(i, ele) {
                ele();
            });
        }
    });
