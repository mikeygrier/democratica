        ...
        try {
          if (R_IGNORE_CHILDREN.test(ele.tagName)) {
            ...
            switch (ele.tagName) {
              ...
              case 'U':
              case 'VAR':
                // take note of trailing and leading spaces inside the tag so we can put them outside the markdown
                var leading_spaces = ele.textContent.match(/^\s+/);
                var trailing_spaces = ele.textContent.match(/\s+$/);

                // get rid of leading and trailing whitespace
                ele.textContent = trim(ele.textContent);

                // place the leading spaces outside of the markdown if there are any
                if (leading_spaces != null) {
                  this.output(leading_spaces[0] + "_");
                } else {
                  this.output("_");
                }
                ...
                // place the trailing spaces outside the markdown if there are any
                if (trailing_spaces != null) {
                  after = this.outputLater('_' + trailing_spaces[0]);
                } else {
              ...
              case 'STRONG':
                ...
                // place the trailing spaces outside the markdown if there are any
                if (trailing_spaces != null) {
                  after = this.outputLater('**' + trailing_spaces[0]);
                } else {
              ...
              case 'DD':
                ...
                break;
              case 'A':
                href = encodeURI(this.attr(ele, 'href', this.options.absolute)).replace(/%25/g, '%');
                if (!href) {
                  break;
                }
